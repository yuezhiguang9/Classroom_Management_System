...
public class ClassroomManagerServiceImpl extends ServiceImpl<ClassroomManagerMapper, Classroom_manager> implements ClassroomManagerService {
    ...
    @Override
    public ResultDTO<Boolean> updateStatus(String token, String roomNum, String roomStatus) {
        try {
            // 验证token并获取管理员账号
            String actualToken = JwtUtil.extractToken(token);
            String mgrAccount = JwtUtil.getUserAccountToken(actualToken);
            ...
            // 验证状态值
            if (!"可用".equals(roomStatus) && !"维修中".equals(roomStatus) && !"已占用".equals(roomStatus)) {
                return ResultDTO.fail(400, "无效的教室状态");
            }
            
            // 更新教室状态
            int result = classroomManagerMapper.updateSingleRoomStatus(roomNum, roomStatus, mgrAccount);
            
            if (result > 0) {
                return ResultDTO.success(true, "更新成功");
            } else {
                return ResultDTO.fail(400, "更新失败，请检查教室编号是否在您的管辖范围内");
            }
    ...
    @Override
    public ResultDTO<List<ApplyInfoManageVO>> listApplyInfo(String token, String applyStatus, 
        ...
        try {
            ...
            // 创建分页对象
            Page<ApplyInfoManageVO> pageObj = new Page<>(page, size);
            
            // 查询申请审核情况
            Page<ApplyInfoManageVO> result = classroomManagerMapper.selectApplyInfoByManager(
                pageObj, mgrAccount, applyStatus, dateStart, dateEnd
            );
            
            // 获取管辖教室总数
            Integer managedRooms = classroomManagerMapper.countManagedRooms(mgrAccount);
            
            List<ApplyInfoManageVO> records = result.getRecords();
            
            // 在第一条记录中设置统计信息
            if (!records.isEmpty()) {
                records.get(0).setManaged_rooms(managedRooms);
            }
